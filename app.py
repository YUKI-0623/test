...
def fetch_live_race_data(date_str, place_code, race_num):
    ...
    try:
        ...
        # 【修正ポイント】選択された日付の「年（4桁）」に自動連動するように変更
        ...
        if not race_id:
            # カレンダーから取得できない場合の予備ID生成も選択年に連動
            race_id = f"{date_str[:4]}{place_code}0101{int(race_num):02d}"
        ...
        if not table:
            return None, "⚠️ 出馬表（確定枠順）のデータが見つかりません。ネット競馬側でまだ枠順が発表されていないか、すでに終了した過去のレースの可能性があります。"
            
        rows = table.find_all("tr", class_="HorseList")
        if not rows:
            return None, "⚠️ 出馬表はありますが、まだ出走馬が登録されていません。"
...
with st.sidebar:
    st.header("📅 レース条件の指定")
    # デフォルトは今週末の想定日付（2026年6月21日）
    tgt_date = st.date_input("開催日を選択", datetime(2026, 6, 21))
    tgt_place = st.selectbox("競馬場を選択", list(PLACE_MAP.keys()), index=7) # デフォルト京都
    tgt_race = st.selectbox("レース番号", [f"{i}R" for i in range(1, 13)], index=10) # デフォルト11R
